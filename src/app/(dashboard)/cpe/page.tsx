...
export default function CPEPage() {
  const percentage = (cpeData.currentCredits / cpeData.targetCredits) * 100;
    ...
    <>
      <div className="p-4 md:p-6 pb-16 max-w-[1280px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-xl md:text-2xl font-bold mb-1 tracking-tight text-primary">ระบบสะสมหน่วยกิตการศึกษาต่อเนื่อง (CPE)</h1>
          ...
          <div className="lg:col-span-1 space-y-6">
            <Card className="card-shadow border-t-4 border-t-primary overflow-hidden relative">
              ...
              <CardContent className="p-6 flex flex-col items-center justify-center text-center relative z-10">
                <Badge variant={cpeData.status === 'active' ? 'default' : 'destructive'} className="mb-6 px-3 py-1 shadow-sm">
                  {cpeData.status === 'active' ? 'สถานะปกติ' : 'ใกล้หมดอายุ'}
                </Badge>
                ...
                <div className="relative w-48 h-48 mb-6 flex items-center justify-center">
                  ...
                  <div className="absolute flex flex-col items-center justify-center">
                    <span className="text-4xl font-bold tracking-tighter text-foreground">{cpeData.currentCredits}</span>
                    <span className="text-sm text-muted-foreground font-medium">/ {cpeData.targetCredits} CPE</span>
                  </div>
                </div>
                ...
                <div className="w-full bg-muted/30 p-4 rounded-xl border border-border/50 text-left">
                  <div className="flex justify-between items-center mb-2 border-b border-border/50 pb-2">
                    ...
                    <span className="text-sm font-semibold text-foreground">{cpeData.expiryDate}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-muted-foreground">ระยะเวลาที่เหลือ</span>
                    <span className="text-sm font-semibold text-amber-600 dark:text-amber-500">{cpeData.timeLeft}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="card-shadow">
              ...
              <CardContent className="pt-6">
                <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-border before:to-transparent">
                  {cpeData.history.map((item, index) => (
                      ...
                      <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border bg-card/50 shadow-sm transition-all hover:shadow-md hover:bg-muted/10">
                        ...
                        <div className="mt-3 flex items-center justify-between">
                          <div className="flex items-center gap-1.5 text-primary font-medium">
                            ...
                            <span className="text-sm">{item.credits} CPE</span>
                          </div>
            ...
            <div className="space-y-4">
              <h3 className="text-base font-semibold flex items-center gap-2 px-1">
                ...
                แนะนำกิจกรรมเก็บหน่วยกิต (คุณขาดอีก {cpeData.targetCredits - cpeData.currentCredits} CPE)
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {cpeData.recommended.map((item) => (
                  <Card key={item.id} className="group overflow-hidden border-muted-foreground/20 hover:border-primary/50 transition-colors cursor-pointer">
                    <CardContent className="p-0">
                      <div className="p-4 bg-gradient-to-br from-card to-muted/30">
                        <div className="flex justify-between items-start mb-3">
                          ...
                          <Badge variant="secondary" className="font-bold">+{item.credits} CPE</Badge>
                        </div>
                      ...
                      <div className="px-4 py-3 bg-muted/50 border-t flex justify-between items-center">
                        <span className="text-xs font-medium text-muted-foreground group-hover:text-foreground transition-colors">ดูรายละเอียด</span>
                        <span className="material-symbols-outlined text-sm text-muted-foreground group-hover:text-primary transition-colors group-hover:translate-x-1">arrow_forward</span>
                      </div>
                    </CardContent>
                  </Card>
                ))}
